...
class ASDLGrammar(object):
    ...
    def __init__(self, productions, root_type=None):
        ...
        self._production_ids = OrderedDict()
        # self._constructor_production_map = dict()
        for prod in productions:
            if prod.type not in self._production_ids:
                self._production_ids[prod.type] = list()
            self._production_ids[prod.type].append(self.prod2id[prod])
    ...
    @staticmethod
    def from_text(text, root_type=None):
        def _parse_field_from_text(_text):# 返回Field
            d = _text.strip().split(' ')
            name = d[1].strip()
            type_str = d[0].strip()
            # field = type* name
            ...
            if type_str in origin_primitive_type_names:
                if type_str in origin_map:
                    field = Field(name, ASDLPrimitiveType(type_str), cardinality=cardinality, value=origin_map[name])
                else:
                    field = Field(name, ASDLPrimitiveType(type_str), cardinality=cardinality, value=name.strip('leaf_'))
            elif type_str in var_primitive_type_names:
                field = Field(name, ASDLPrimitiveType(type_str), cardinality=cardinality)
            else:
                field = Field(name, ASDLCompositeType(type_str), cardinality=cardinality)
            
            if field.target_value is not None:
                if field.target_value == 'class':
                    field.target_value = tokenizer(field.target_value)['input_ids'][1:-1]
                else:
                    field.target_value = tokenizer(' ' + field.target_value)['input_ids'][1:-1]
            return field

        def _parse_constructor_from_text(_text):# 对constructor做处理,返回ASDLConstructor （type name, type name） type leaf::type
            _text = _text.strip()
            # fields = None
            field_blocks = [type.replace('leaf::', 'leaf_') + ' ' + type.replace('leaf::', 'leaf_') for type in _text.split(' ')]
            fields = map(_parse_field_from_text, field_blocks)
            # if '(' in _text:
            #     # 得到构造器的名称
            #     name = _text[:_text.find('(')]
            #     # 得到构造器的fields
            #     field_blocks = _text[_text.find('(') + 1:_text.find(')')].split(',') # [type name, type name]
            #     fields = map(_parse_field_from_text, field_blocks)
            # else:
            #     name = _text
        ...
        # 读入xxx_asdl.txt,删掉注释，按行划分
        lines = remove_comment(text).split('\n')
        # 删掉句头句尾多余的空格
        # map是对每一元素做自定义的映射，filter是对序列每个元素过滤
        ...
        # 读取第一行的基本类型
        origin_primitive_type_names = list(map(lambda x: 'leaf_'+x.strip(), lines[line_no].split(' ')))
        origin_map = {
            'leaf_boolean_type': 'boolean',
            'leaf_void_type': 'void',
            'leaf_string_literal': 'srini_string',
            'leaf_null_literal': 'null',
            # 'ERROR': "\x00",
            'leaf_ERROR': "\x00",
            'leaf_endif_directive': "endif",
            'leaf_preproc_integer_literal': '612',
            'leaf_interpolated_string_text': '@',
            'leaf_global': '',
            'leaf_parameter_modifier': 'this',
            'leaf_preproc_string_literal': '( '
        }
        line_no += 1
        var_primitive_type_names = list(map(lambda x: 'leaf_'+x.strip(), lines[line_no].split(' ')))
        line_no += 1
        ...
        while True:
            ...
            # 以等号分割成两半并且删掉了句头句尾空格，得到production左侧的type
            type_name = type_block[:type_block.find('-->')].strip()
            # 得到右侧的构造器list
            constructors_blocks = type_block[type_block.find('-->') + 3:].split('|')
            i = line_no + 1
            # 继续处理下一行的同句
            while i < len(lines) and lines[i].strip().startswith(type_name + '-->'):
                t = lines[i].strip()
                # 删掉开头的|
                cont_constructors_blocks = [t.split('-->')[1]]
                # 列表合并
            ...
            constructors = map(_parse_constructor_from_text, constructors_blocks)# ASDLConstructor

            productions = list(map(lambda c: ASDLProduction(new_type, c), constructors))# 每一个constructor都对应一个production
        ...
        # all_productions前面加一个pad_production
        # all_productions = ['pad_production'].extend(all_productions)
        grammar = ASDLGrammar(all_productions, root_type)# grammar是production的集合
        print("打印grammar\n")
        print(str(grammar))
        return grammar

# Production: type = constructor([Fields])
...
# Constructor: constructorName([Fileds])
class ASDLConstructor(object):
    ...
    def __repr__(self, plain=False):
        plain_repr = '%s(%s)' % (self.name,
                                 ' '.join(f.__repr__(plain=True) for f in self.fields)) # fields是str类型而不是Field类型
        if plain: return plain_repr
        else: return 'Constructor(%s)' % plain_repr

# Filed = type* name
class Field(object):
    def __init__(self, name, type, cardinality, value=None):
        self.name = name
        self.type = type
