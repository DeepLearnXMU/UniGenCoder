...
def java_ast_to_asdl_ast(java_ast_node, grammar):
    ...
    production_str = py_node_name + ' -> ' + "None(" + ' '.join(['leaf_' + child.type + ' leaf_' + child.type if child.child_count==0
...
def check(grammar, cursor, out, code, file_name, index,  dict_blank=None, dict_=None, list_no_win=None, tokenizer=None):
    ...
    # assert code == code1
    # assert code == code2
    if ''.join(code.split(' ')) != ''.join(code1.split(' ')) or ''.join(code.split(' ')) != ''.join(
            code2.split(' ')):
        if out!=None:
            out.write(file_name + ' ' + str(index))
            out.write('\n')
        print('\ncode={}, \ncode1={}\ncode2={}'.format(code, code1, code2))
        # return None
...
def run(args):
    ...
    elif args.task == 'translate' and args.lang == 'cs':
    # elif args.task == 'translate' and args.sub_task == 'java-cs':
        grammar_txt = root_path + 'cs_asdl_translate.txt'
    ...
    for file_name, new_file_name in zip(file_list, new_file_list):
        ...
        if args.train:
            ...
            new_file = '/'.join(file_name.split('/')[:-1])
            new_file += '/debug_' + file_name.split('/')[-1]
        ...
        data = data[:100] if not args.train else data
        for index, line in enumerate(tqdm(data)):
            ...
            if args.task == 'concode':
                ...
                json_line = {'code': line.strip()}
            # if args.lang == 'java':
            code = 'class c { ' + json_line['code'] + ' }'
            # else:
                # code = json_line['code']
            tree = parser.parse(bytes(code, "utf8"))
            cursor = tree.walk()
            ...
            code = example['tgt_canonical_code'][9:-1].strip()
            tree_tgt_code = example['tree_tgt_code'][9:-1].strip()
            if args.task == 'concode':
                json_line['code'] = tree_tgt_code
                write_line = json.dumps(json_line)
            else:
                write_line = tree_tgt_code
            new_file.write(write_line+'\n')
        ...
        # 打印分布的bins
        node_sum_counter = Counter(node_sum)
        # sort
