...
class GenTokenAction(Action):
    ...
    def is_stop_signal(self):
        # return self.token == '</primitive>' -> '</s>'. whose id = 2
        return self.token == 2 or self.token == '</s>'
        ...
        for field in asdl_ast.fields:
            ...
            if self.grammar.is_composite_type(field.type):
                if field.cardinality == 'single':
                    field_actions = self.get_actions(field.value)# 递归
                else:
                    field_actions = []
                    ...
                    if field.value is not None:
                        if field.cardinality == 'multiple':
                            for val in field.value:
                                cur_child_actions = self.get_actions(val)# 递归
                ...
                field_actions = self.get_primitive_field_actions(field)# 转到lambda_dcs_transition_system.py get_primitive_field_actions
