...
def convert_target_to_ids(item):
    seq_target, tree_target, tokenizer, args = item
    seq_target = seq_target.replace('</s>', '<unk>')
    seq_target_ids = tokenizer.encode(seq_target, max_length=args.max_target_length, padding='max_length', truncation=True)
    assert seq_target_ids.count(tokenizer.eos_token_id) == 1
    tree_target_ids = tokenizer.encode(tree_target, max_length=args.max_target_length, padding='max_length', truncation=True)
    assert tree_target_ids.count(tokenizer.eos_token_id) == 1

    return_obj = {
        'seq_target_ids': seq_target_ids,
        'tree_target_ids': tree_target_ids
    }
