...
def main():
    ...
    if args.do_train and (args.avg_checkpoint_path is None or not os.path.exists(args.avg_checkpoint_path)):
        ...
        for cur_epoch in range(args.start_epoch, int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(bar):
                ...
                lambda_ = 0.5 - abs(cur_epoch-10)/20
                if lambda_ < 0:
                    lambda_ = 0
            ...
            torch.cuda.empty_cache()
                ...
                # save each epoch's checkpoint
            if args.save_last_checkpoints:
                last_output_dir = os.path.join(args.output_dir, 'checkpoint-last')
                if not os.path.exists(last_output_dir):
                    os.makedirs(last_output_dir)
                model_to_save = student_model.module if hasattr(student_model, 'module') else student_model
                output_model_file = os.path.join(last_output_dir, str(cur_epoch)+".bin")
    ...
    if args.do_eval:
        ...
        # eval each epoch's cached model
        eval_examples, eval_data = load_and_cache_gen_data(args, args.dev_filename, pool, tokenizer, 'dev',
                                                            only_src=True, is_sample=True)
        not_loss_dec_cnt = args.patience + 1
        last_output_dir = os.path.join(args.output_dir, 'checkpoint-last')
        wait_count = 0
        stop_eval = False
        for cur_epoch in range(args.start_epoch, int(args.num_train_epochs)):
            cur_epoch_model_path = os.path.join(last_output_dir, str(cur_epoch)+".bin")
            while not os.path.exists(cur_epoch_model_path):
                logger.info("Wait for %s", cur_epoch_model_path)
                wait_count += 1
                time.sleep(60)
                if wait_count > 10:
                    stop_eval = True
                    break
            if stop_eval:
                logger.info("Stop eval as %s does not exist", cur_epoch_model_path)
                break
            student_model.load_state_dict(torch.load(cur_epoch_model_path))
