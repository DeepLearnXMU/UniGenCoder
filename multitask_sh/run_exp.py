...
def get_cmd(task, sub_task, model_tag, gpu, data_num, bs, lr, source_length, target_length, patience, epoch, warmup,
            ...
            gradient_accumulation_steps = 1, decoder_type = 'seq', load_model_path = None, seed=1234, avg_checkpoint_path=None):
    if max_steps is None:
        cmd_str = 'bash multitask_sh/exp_with_args.sh %s %s %s %d %d %d %d %d %d %d %d %d %s %s %s %d %s %s %d %s' % \
                  (task, sub_task, model_tag, gpu, data_num, bs, lr, source_length, target_length, patience, epoch,
                   warmup, model_dir, summary_dir, res_fn, gradient_accumulation_steps, decoder_type, load_model_path, seed, avg_checkpoint_path)
    else:
        cmd_str = 'bash multitask_sh/exp_with_args.sh %s %s %s %d %d %d %d %d %d %d %d %d %s %s %s %d %d %d' % \
                  (task, sub_task, model_tag, gpu, data_num, bs, lr, source_length, target_length, patience, epoch,
...
def get_args_by_task_model(task, sub_task, model_tag):
    ...
    if task == 'concode':
        ...
        lr = 2
    return bs, lr, src_len, trg_len, patience, epoch, gradient_accumulation_steps


def run_one_exp(args):
    bs, lr, src_len, trg_len, patience, epoch, gradient_accumulation_steps = get_args_by_task_model(args.task, args.sub_task, args.model_tag)
    print('============================Start Running==========================')
    cmd_str = get_cmd(task=args.task, sub_task=args.sub_task, model_tag=args.model_tag, gpu=args.gpu,
                      ...
                      gradient_accumulation_steps = gradient_accumulation_steps, decoder_type=args.decoder_type,
                      load_model_path = args.load_model_path,
                      seed = args.seed,
                      avg_checkpoint_path = args.avg_checkpoint_path)
    print('%s\n' % cmd_str)
    os.system(cmd_str)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--decoder_type', type=str, required=True, choices=['seq', 'tree'])
