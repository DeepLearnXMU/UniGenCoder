...
cmd="CUDA_VISIBLE_DEVICES=${gpu} python run_multitask_distill.py \
...
--train_batch_size 16 --eval_batch_size 10 \
--max_source_length 320 --max_target_length ${trg} \
--gradient_accumulation_steps 2 --decoder_type tree \
--distill_type cross \
--tree_path ${tree_teacher_path} \
--seq_path ${seq_teacher_path} \
--seed ${SEED} \
